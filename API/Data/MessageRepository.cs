...
namespace API.Data
{
    public class MessageRepository : IMessageRepository
    {
       ...
       public async Task<Message> GetMessage(int id)
        ...
        public async Task<IEnumerable<MessageDto>> GetMessageThread(string currentUsername, string recpientName)
        {
           var messages = await _context.Messages
           .Include(u=> u.Sender).ThenInclude(P=> P.Photos)
           .Include(u=> u.Recipient).ThenInclude(p=> p.Photos)
           .Where(m => m.Recipient.UserName==currentUsername
           && m.Sender.UserName==recpientName ||
           m.Recipient.UserName==recpientName 
        ...
        public async Task<PagedList<MessageDto>> GetMessagesForUser(MessagesParams messagesParams)
        {
            ...
            query = messagesParams.Container switch
             {
                "Inbox" => query.Where(u => u.Recipient.UserName == messagesParams.Username 
                    && u.RecipientDeleted == false),
                "Outbox" => query.Where(u => u.Sender.UserName == messagesParams.Username
                    && u.SenderDeleted == false),
                _ => query.Where(u => u.Recipient.UserName ==
                    messagesParams.Username && u.RecipientDeleted == false && u.DateRead == null)
            };
